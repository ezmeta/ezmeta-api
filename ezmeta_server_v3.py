...
# ============================================================
# EZMETA API SERVER v2.0 — Multi-Client Support
# Run: python3 -X utf8 -m uvicorn ezmeta_server_v2:app --reload --port 8888
# ============================================================
...
def root():
    ...
    return {
        "status": "EZMeta API v2.0 Running",
    ...
    for i, c in enumerate(data["clients"]):
        if c["id"] == client_id:
            for field in allowed_fields:
                if field in updates:
                    data["clients"][i][field] = updates[field]
            data["clients"][i]["last_updated"] = datetime.now().isoformat()
            save_clients(data)
...
def change_password(client_id: str, old_password: str, new_password: str):
    ...
    for i, c in enumerate(data["clients"]):
        if c["id"] == client_id:
            ...
            data["clients"][i]["temp_password"] = ""  # clear temp password
            save_clients(data)
