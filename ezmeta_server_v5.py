...
app = FastAPI(title="EZMeta API v5.0", version="5.0.0")
...
def fetch_insights(token, campaign_id, date_preset="last_7d"):
    """Fetch insights for a campaign. date_preset: today, yesterday, last_7d, this_month"""
...
def fetch_insights_daily(token, campaign_id, days=7):
    ...
    params = {
        "fields": "spend,ctr,roas,frequency,impressions,clicks,actions,action_values",
        "time_increment": 1,
...
def build_campaigns_from_meta(token, account_id):
    ...
    campaigns = []
    for c in raw:
        # Fetch 3 periods: today, last 7 days, this month
        ins_today    = fetch_insights(token, c["id"], date_preset="today")
        ins_7d       = fetch_insights(token, c["id"], date_preset="last_7d")
        ins_month    = fetch_insights(token, c["id"], date_preset="this_month")
        daily_data   = fetch_insights_daily(token, c["id"], days=7)
        ...
        def parse_ins(ins):
            ...
            for action in ins.get("actions", []):
                if action["action_type"] in ["purchase", "lead", "omni_purchase"]:
            ...
            return spend, clicks, impressions, ctr, cpc, frequency, conversions, conv_value

        spend_today, clicks_today, imp_today, ctr_today, cpc_today, freq_today, conv_today, val_today = parse_ins(ins_today)
        spend_7d,    clicks_7d,    imp_7d,    ctr_7d,    cpc_7d,    freq_7d,    conv_7d,    val_7d    = parse_ins(ins_7d)
        spend_month, _,            _,         _,         _,         _,          conv_month, val_month = parse_ins(ins_month)

        roas_today = round(val_today / spend_today, 2) if spend_today > 0 else 0
        roas_7d    = round(val_7d / spend_7d, 2) if spend_7d > 0 else 0

        # Build daily arrays from real daily breakdown
        def build_daily_arr(field, fallback):
            arr = []
            for d in daily_data:
                arr.append(round(float(d.get(field, fallback)), 2))
            while len(arr) < 7:
                arr.insert(0, fallback)
            return arr[-7:]

        spend_7d_arr = [round(float(d.get("spend", 0)), 2) for d in daily_data][-7:]
        ctr_7d_arr   = [round(float(d.get("ctr", ctr_7d)), 2) for d in daily_data][-7:]
        freq_7d_arr  = [round(float(d.get("frequency", freq_7d)), 2) for d in daily_data][-7:]
        ...
        for d in daily_data:
            ...
            for av in d.get("action_values", []):
                if av["action_type"] in ["purchase", "omni_purchase"]:
                    v += float(av["value"])
            roas_7d_arr.append(round(v / s, 2) if s > 0 else 0)
        roas_7d_arr = roas_7d_arr[-7:] if roas_7d_arr else [roas_7d] * 7

        # Pad arrays to 7 items
        while len(spend_7d_arr) < 7: spend_7d_arr.insert(0, 0)
        while len(ctr_7d_arr)   < 7: ctr_7d_arr.insert(0, ctr_7d)
        while len(freq_7d_arr)  < 7: freq_7d_arr.insert(0, freq_7d)
        while len(roas_7d_arr)  < 7: roas_7d_arr.insert(0, roas_7d)
        ...
        campaigns.append({
            ...
            "monthly_spend": int(spend_month * 100),
            "spend_today":   int(spend_today * 100),
            "spend_7d":      spend_7d_arr,
            "roas":          roas_today if roas_today > 0 else roas_7d,
            "roas_7d":       roas_7d_arr,
            "ctr":           round(ctr_today if ctr_today > 0 else ctr_7d, 2),
            "ctr_7d":        ctr_7d_arr,
            "cpc":           int((cpc_today if cpc_today > 0 else cpc_7d) * 100),
            "clicks":        clicks_today if clicks_today > 0 else clicks_7d,
            "impressions":   imp_today if imp_today > 0 else imp_7d,
            "frequency":     round(freq_today if freq_today > 0 else freq_7d, 1),
            "frequency_7d":  freq_7d_arr,
            "conversions":   conv_today if conv_today > 0 else conv_7d,
            "cpm":           int(float(ins_7d.get("cpm", 0)) * 100) if ins_7d else 0,
        })
...
def root():
    ...
    return {
        "status": "EZMeta API v5.0 Running",
...
def scan_all_clients():
    ...
    # BOT_TOKEN — ambil dari env variable terus (Railway), fallback ke config
    ...
    for client in clients_data["clients"]:
        ...
        mode = "LIVE" if token and token != "LETAK_TOKEN_SINI" and account_id else "DEMO"
...
@app.get("/scan/status")
def scan_status():
    """Check status semua clients — ada token, ada chat_id ke tak"""
    clients_data = load_clients()
    ...
    for client in clients_data["clients"]:
        ...
        results.append({
            "client_id": client["id"],
            "client_name": client["name"],
            "has_token": bool(token and token != "LETAK_TOKEN_SINI"),
